...
for ite in range(20):
  ...
  for i,v in enumerate(labels_train_gender):
      ...
      final_label_train.append([labels_train_gender[i],labels_train_moustache[i],labels_train_glass[i]])
  ...
  for i,v in enumerate(labels_test_genders):
      ...
      final_label_test.append([labels_test_genders[i],labels_test_moustaches[i],labels_test_glasss[i]])
  ...
  file = open('/home/ubuntu/BTP_git/log2big.txt','a')

  model = big_XCEPTION(input_shape, num_classes)

  if ite:
    model.load_weights("/home/ubuntu/BTP_git/weights/test2big/"+"model_%d.h5"%(ite-1))
  ...
  file.write("*****************"+ "\n")
  model.save_weights("/home/ubuntu/BTP_git/weights/test2big/"+"model_%d.h5"%ite)
